...
//////////////////////////////
//
// goToNextNoteOrRest -- current is the value of global variable window.CursorNote.
...
export function markItem(item, line) {
	if (!item) {
		item = window.CursorNote;
	}
	if (!item) {
		return;
	}
	window.EditorLine = line;
	// This case is not good for editing a note:
	//if (window.CursorNote && item && (window.CursorNote.id == item.id)) {
	//	console.log("THE SAME NOTE");
	//	return;
	//}
	if (window.CursorNote) {
		// console.log("TURNING OFF OLD NOTE", window.CursorNote);
		/// window.CursorNote.setAttribute("fill", "#000");
		// window.CursorNote.removeAttribute("fill");

		var classes = window.CursorNote.getAttribute("class");
		var classlist = classes.split(" ");
		var outclass = "";
		for (var i=0; i<classlist.length; i++) {
			if (classlist[i] == "highlight") {
		...
		window.CursorNote.setAttribute("class", outclass);
	...
	if (window.CursorNote) {
		// console.log("TURNING ON NEW NOTE", window.CursorNote);
		// window.CursorNote.setAttribute("fill", "#c00");
