...
export function getMusicalParameters(note) {
  ...
  const indexOfContainingStaff = [...containingMeasure.children].findIndex(
    (child) => child.id == note.closest('.staff').id
  );
  const staff =
    [...containingMeasure.children]
      .filter((child) => child.classList.contains('staff'))
      .slice(0, indexOfContainingStaff).length + 1;

  let order;
  const containingBeam = note.closest('.beam');
  if (containingBeam) {
    const indexOfNoteInBeam = [...containingBeam.children].findIndex(
      (child) => child.id == note.id
    );
    order = [...containingBeam.children]
      .filter(
        (child) =>
          child.classList.contains('note') || child.classList.contains('chord')
      )
      .slice(0, indexOfNoteInBeam).length;
    note = containingBeam;
  } else order = 1;

  const container = note.closest('.layer') ?? note.closest('.staff');
  const indexOfNoteInContainer = [...container.children].findIndex(
    (child) => child.id == note.id
  );
  const siblings = [...container.children].slice(0, indexOfNoteInContainer);
  order += siblings.filter(
    (sibling) =>
      sibling.classList.contains('note') || sibling.classList.contains('chord')
  ).length;
  siblings
